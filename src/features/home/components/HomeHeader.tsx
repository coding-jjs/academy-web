    ...
    return (
        <header className={styles.header}>
            ...
            <div className={styles.authActions}>{viewer ? <>
                ...
                <Link href={viewer.dashboardHref} className={cx(styles.authButton, styles.primaryAuthButton, styles.dashboardButton)}>대시보드 <span aria-hidden="true">→</span></Link><LogoutButton className={cx(buttonStyles.cancel, styles.authButton, styles.logoutButton)} />
            </> : <><Link href="/login" className={cx(styles.authButton, styles.loginButton)}>로그인</Link><Link href="/signup" className={cx(styles.authButton, styles.primaryAuthButton)}>회원가입</Link></>}</div>
        </header>
    );
}

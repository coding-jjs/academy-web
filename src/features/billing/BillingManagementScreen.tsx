...
export default function BillingManagementScreen({ students, invoices, canManage, deniedMessage }: { students: BillingStudentOption[]; invoices: BillingInvoiceRow[]; canManage: boolean; deniedMessage?: string }) {
    ...
    if (!canManage) return <section className={styles.page}><BillingHeading description={deniedMessage ?? "결제/청구 관리 권한이 없습니다."} /></section>;
    return (
...
function BillingHeading({ description }: { description: string }) { return <header className={styles.heading}><div><span>BILLING</span><h1>청구·수납</h1><p>{description}</p></div></header>; }
